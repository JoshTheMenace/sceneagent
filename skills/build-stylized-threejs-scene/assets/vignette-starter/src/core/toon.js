 ...
 *
 * Ported from Sakura Crossing's src/core/toon.js; the only scene-specific
 * value it carried was the default shadow tint, kept here as DEFAULTS.
 * ------------------------------------------------------------------ */

const DEFAULTS = {
  // cool violet; a scene overrides per-material via cel({ tint }) or keys it
  // off its palette's shadowTint role
  shadowTint: 0x6c5f8c,
};
...
let patchedChunk = '';
{
  ...
  if (src && src.includes(TOON_LINE)) {
    ...
    // the violet shadow tint is most of what separates cel from low-poly 3D;
    // losing it silently on a three.js upgrade must not be possible
    console.error(
      `[toon] shadow-tint patch failed: three.js r${THREE.REVISION} no longer contains ` +
      `the expected ${TOON_CHUNK} line. Cel shading will render WITHOUT the shadow tint. ` +
      'Pin three to the version this repo was built against, or update TOON_LINE.'
    );
  }
}

/** True when the shadow-tint shader patch applied — smoke tests should assert this. */
export const shadowTintActive = () => patchAvailable;
 ...
 * the whole scene ends up sharing a few dozen shader programs.
 */
export function cel(opts = {}) {
  const {
    color = 0xffffff,
    bands = 3,
    tint = DEFAULTS.shadowTint,
    flat = true,
    map = null,
    emissive = null,
    emissiveIntensity = 1,
